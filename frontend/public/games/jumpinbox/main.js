...
function update() {
  ...
  color('light_black')
  box(50, (100 - wallSize) / 2 - wallThickness / 2, wallSize + wallThickness * 2, wallThickness)
  box(50, (100 + wallSize) / 2 + wallThickness / 2, wallSize + wallThickness * 2, wallThickness)
  box((100 - wallSize) / 2 - wallThickness / 2, 50, wallThickness, wallSize + wallThickness * 2)
  box((100 + wallSize) / 2 + wallThickness / 2, 50, wallThickness, wallSize + wallThickness * 2)
  color('light_blue')
  remove(trails, (t) => {
  ...
  if (player.wallSide != null) {
    ...
    color('blue')
    line(player.pos, vec(player.pos).addWithAngle(player.angle + player.wallSide, 20))
  ...
  if (nextGoldPos.x < (100 - wallSize) / 2 || nextGoldPos.x > (100 + wallSize) / 2) {
    nextGoldVel.x *= -1
    nextGoldVel.y += rnds(0.2)
    nextGoldVel.normalize()
  }
  if (nextGoldPos.y < (100 - wallSize) / 2 || nextGoldPos.y > (100 + wallSize) / 2) {
    nextGoldVel.y *= -1
...
function moveGold(g) {
  ...
  if (g.isRed && c.black && !c.light_blue) {
    play('explosion')
    if (window.onGameEnd) window.onGameEnd(score)
    end()
    return
  }
  ...
  if (g.pos.isInRect(-g.size.x / 2 + 10, -g.size.y / 2 + 10, 80 + g.size.x, 80 + g.size.y)) {
    g.isInScreen = true
  } else {
    ...
    if (
      g.isInScreen &&
      !g.isRed &&
      !g.pos.isInRect(g.size.x / 2, g.size.y / 2, 100 - g.size.x, 100 - g.size.y)
    ) {
      play('laser')
      g.isRed = true
      g.vel.rotate(PI)
