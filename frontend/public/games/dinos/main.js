...
function update() {
  ...
  if (player.jumpPhase === 0) {
    let targetSpeed =
      player.powerUpTicks > 60 && player.multiplier > 0 ? 2 + player.multiplier * 0.3 : 1
    ...
    player.pos.y += player.vel.y
    if ((player.pos.y > 75 && player.gravity > 0) || (player.pos.y < 23 && player.gravity < 0)) {
      play('stomp', { volume: 9 })
      player.jumpPhase = 0
  ...
  const isPowerUp =
    ...
    (player.jumpPhase === 2 &&
      ((player.gravity === 1 && player.pos.y > 50) || (player.gravity === -1 && player.pos.y < 50)))
  char(addWithCharCode('d', [0, 1, 2, 1][floor(player.animIndex * 0.1) % 4]), player.pos, {
    edgeColor: isPowerUp ? (player.powerUpTicks < 45 ? 'yellow' : 'red') : 'white',
  ...
  remove(cactuses, (c) => {
    ...
    const cc = box(c.pos.x, c.pos.y + groundY[c.gravity > 0 ? 0 : 1], 3, 9).isColliding.char
